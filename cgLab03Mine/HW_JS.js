...
myScene.add(axes);  
myScene.background=new THREE.Color('0xff00ff');
...
cubeGeo.setAttribute('uv',new THREE.Float32BufferAttribute(uvs,2)); //three js 내부적으로 shader 제공. 이 naming 따라 buffer 쓰겠다.
cubeGeo.computeVertexNormals();


const myGeo=new THREE.BoxGeometry(10,10,10);
console.log(myGeo.getAttribute('position')); //6 vtx * 4 = 24
// console.log(cubeGeo.getAttribute('uv')); 

//*texture img mapping & mtr setting
const textureLoader=new THREE.TextureLoader();
const myTex=textureLoader.load('./lab5.jpeg');

const cubeMaterial=new THREE.MeshPhongMaterial( { color: '#CFF5E7', wireframe:false, map: myTex } );
//img 뒤집어진 이유: txt 시작방향 ==좌측 하단인데 uv (0,0)을 좌측 상단에 잡았기 때문에
//뒷면 안 그려진 이유 : 빛이 없어서 -> ambient Light 별개 소스로 쓰도록 하나 더 추가하기
//img 안 늘어지게 하려면 vtx 최소화해서 triangle 처리했는데, 면 단위로 vtx 지정되도록 -> geo boxGeo로 set :72 Line
let myMesh = new THREE.Mesh( myGeo, cubeMaterial );
myScene.add(myMesh);
...
const myAmbLight=new THREE.AmbientLight(0x404040,0.5);
myScene.add(myAmbLight);
